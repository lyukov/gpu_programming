...
inline void _processOneLine(
        int x,
        uint8* inputImage, uint8* outputImage,
        int height, int width, int channels,
        int radius, uint8* arr
) {
...
void MedianFilterCPU(uint8* inputImage, uint8* outputImage, int height, int width, int channels) {
    ...
    for (int x = 0; x < width; ++x) {
        _processOneLine(x, inputImage, outputImage, height, width, channels, radius, arr);
    }
}
